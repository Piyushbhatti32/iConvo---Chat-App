      ...
      <div class="room-link" onclick="joinRoom('${escapeHtml(room.name)}')">
        <div class="room-info">
          ...
          <div class="room-details">
            ...
            <div class="room-users">${room.users} users • ${
      room.messages
    } messages</div>
          </div>
        </div>
...
function joinRoom(roomName) {
    ...
    try {
        ...
        if (currentRoom === roomName) {
            ...
            showFeedbackMessage("You are already in this room");
            closeSidebar(); // Close sidebar even when room join is rejected
            return;
        }
        ...
        // Show loading state
        chatRoom.innerHTML = `Chatroom: <span>Joining ${escapeHtml(roomName)}...</span>`;
...
function initializeEmojiPicker() {
    try {
        ...
        document.addEventListener('click', (e) => {
            if (!rootElement.contains(e.target) && !emojiButton.contains(e.target)) {
                rootElement.style.display = 'none';
            }
        });
